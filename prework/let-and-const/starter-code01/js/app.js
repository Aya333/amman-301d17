...
function displayPics(){
  ...
  // TODO: In a sentence or two, explain why the previous line of code threw an error when we changed the let declaration from `const to `const`.
  // Because the (Var) is a global identifier so the variable is glabally identified unlike (let) which doesn't identify the variable glbally that's why it gave an error because the console couldn't reach the variable
  // For the const it gave an error when we changed the variable type is because once you assign a value to const you can't change as if the assigned value is permenant
  console.log(viewed);
